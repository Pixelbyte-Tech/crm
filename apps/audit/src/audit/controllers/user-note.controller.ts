...
import { KafkaDlq, UserNoteCreatedEvent, UserNoteDeletedEvent, UserNoteUpdatedEvent } from '@crm/kafka';
...
export class UserNoteController {
   ...
   */
  @KafkaDlq({ topic: UserNoteCreatedEvent.type })
  @EventPattern([UserNoteCreatedEvent.type, `${UserNoteCreatedEvent.type}.retry`])
  async onUserCreated(@Payload() e: UserNoteCreatedEvent, @Ctx() context: KafkaContext): Promise<void> {
    const msg = `Received ${UserNoteCreatedEvent.name} for '${e.data?.noteId ?? 'n/a'}'`;
    this.#logger.log(`${msg}`);
    ...
    await this.auditService.persist(
      {
        ...
        targetType: AuditTarget.NOTE,
        targetId: e.data.noteId,
   ...
   * @param context The kafka context
   */
  @KafkaDlq({ topic: UserNoteUpdatedEvent.type })
  @EventPattern([UserNoteUpdatedEvent.type, `${UserNoteUpdatedEvent.type}.retry`])
  async onUserUpdated(@Payload() e: UserNoteUpdatedEvent, @Ctx() context: KafkaContext): Promise<void> {
    const msg = `Received ${UserNoteUpdatedEvent.name} for '${e.data?.noteId ?? 'n/a'}'`;
    this.#logger.log(`${msg}`);
   ...
   * @param context The kafka context
   */
  @KafkaDlq({ topic: UserNoteDeletedEvent.type })
  @EventPattern([UserNoteDeletedEvent.type, `${UserNoteDeletedEvent.type}.retry`])
  async onUserDeleted(@Payload() e: UserNoteDeletedEvent, @Ctx() context: KafkaContext): Promise<void> {
    const msg = `Received ${UserNoteDeletedEvent.name} for '${e.data?.noteId ?? 'n/a'}'`;
    this.#logger.log(`${msg}`);
